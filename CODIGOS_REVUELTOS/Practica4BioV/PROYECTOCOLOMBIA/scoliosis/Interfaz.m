...
if S == 1
    ...
    dimensionmedioy=round(y1/2)+valory;
    ...
    angulo=atand((pos2x+pos1x)/(pos2y-pos1y));
    cadena= strcat('Angulo: ',int2str(abs(angulo-90)));
...
elseif S==2
    ...
    valorx = str2num(get(handles.paciente,'String'));
    valory = str2num(get(handles.ejey,'String'));
    dimensionmediox=round(x1/2)+valorx-200;
    dimensionmedioy=round(y1/2)+valory-120;
    
    imgposterior=imcrop(imgposterior,[dimensionmedioy,dimensionmediox,640,480]);
    %descomentar linea de arriba para que funcione con imagenes pasadas
